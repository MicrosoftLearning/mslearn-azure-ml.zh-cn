...
1. 请注意“管理”部分，其中包括“计算”和“数据存储”等  。 这些是训练或部署机器学习模型所需的基础结构资源。 
...
> [!Note]
> 弹出窗口可能会出现在整个过程中，以引导你完成整个工作室。 可以关闭并忽略所有弹出窗口，并专注于本实验室的说明。

1. 从工作室左侧的菜单中选择“设计器”页面。
1. 选择“Regression - Automobile Price Prediction (Basic)”示例。 
    
    此时会显示一个新管道。 管道顶部显示了一个组件，用于加载“Regression - Automobile Price Prediction (Basic)”。 管道处理数据并训练线性回归模型，以预测每辆汽车的价格。
1. 选择页面顶部的“提交”。 出现错误，因为尚未选择计算目标。 没有计算资源，管道无法运行。 
...
1. 在 Azure 机器学习工作室中，从左侧菜单中选择“计算”页面。 可以使用四种类型的计算资源：
    ...
    - **推理群集**：推理期间使用的 Kubernetes 群集。 非常适合大规模实时模型部署。
...
1. 导航到“设计器”页。
1. 选择“经典预生成”选项卡。
1. 选择“Regression - Automobile Price Prediction (Basic)”管道草稿。
1. 选择右上角的“设置”以展开“设置”窗格 。
1. 在“选择计算类型”下选择“计算实例” 。
1. 在“选择 Azure ML 计算实例”下选择新创建的计算实例。 
1. 选择“提交”再次运行训练管道。
1. 将出现一个弹出窗口，用于设置管道作业。 使用以下设置配置并提交新的管道作业：
    - 试验：`Create new`
    - 新试验名称：`train-regression-designer`
    - 保留所有其他默认设置。
